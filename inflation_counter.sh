...
initial_year=1955 # used for calculating index offsets, not actually a year
...
starting_index=$(($starting_year - $initial_year + 1)) # +1 because that year's inflation didn't affect the price
ending_index=$(($ending_year - $initial_year))
for i in $(seq $starting_index $ending_index); do
  percent=$(echo $georgia | cut -d "," -f"$i" | sed 's/\"//g')
  year=$(echo $key | cut -d "," -f"$i" | sed 's/\"//g')
  price_calc="scale=6; $ending_price + ($ending_price "'*'" $percent / 100)"
  ending_price=$(bc <<< "$price_calc")
  echo $year $percent $ending_price
done
